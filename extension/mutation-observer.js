            ...
            if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
                for (const node of mutation.addedNodes) {
                    ...
                    if (node.nodeType === Node.ELEMENT_NODE) {
                        ...
                        const textFields = node.querySelectorAll('input, textarea, [contenteditable="true"], [role="textbox"], [role="combobox"], [role="searchbox"]');
                        for (const textField of textFields) {
                            textField.addEventListener('input', handleInput);
                            textField.addEventListener('keydown', handleKeydown);
                            textField.addEventListener('focusin', handleFocusIn);
                        }
                        
                        // Special handling for WhatsApp
                        if (window.location.hostname.includes('web.whatsapp.com')) {
                            const whatsappInputs = node.querySelectorAll('[data-testid="conversation-compose-box-input"], [data-tab="6"], [title="Type a message"]');
                        ...
                        if (window.location.hostname.includes('discord.com')) {
                            const discordInputs = node.querySelectorAll('[class*="slateTextArea"], [class*="editor-"], [class*="channelTextArea-"]');
