...
// Add a global click handler to detect clicks on the suggestion overlay
document.addEventListener('click', (event) => {
  // Check if the click target is the suggestion overlay or a child of it
  const target = event.target;
  if (target && (
      target.classList.contains('ghosttyper-suggestion-overlay') ||
      target.closest('.ghosttyper-suggestion-overlay')
  )) {
    console.log('GhostTyper: Global click handler detected click on overlay', { target });

    // Get the suggestion overlay instance
    if (suggestionOverlay && suggestionOverlay.isVisible) {
      ...
      if (suggestion && suggestionOverlay.currentInput) {
        console.log('GhostTyper: Global handler accepting suggestion:', suggestion);

        // Accept the suggestion
        suggestionOverlay.acceptSuggestion(suggestion);

        // Focus back on the input element
        suggestionOverlay.currentInput.focus();
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  console.log('GhostTyper: Initialized via DOMContentLoaded');
});

// Also initialize immediately in case DOMContentLoaded already fired
if (document.readyState === 'interactive' || document.readyState === 'complete') {
  const ghostTyper = new GhostTyper();
  console.log('GhostTyper: Initialized immediately, document.readyState =', document.readyState);
}
